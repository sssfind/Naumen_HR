...
Веб-приложение для HR, наставников и стажёров: план адаптации по блокам и задачам, еженедельная обратная связь с анализом тональности, дашборды рисков, уведомления и встроенный чат с LLM.
...
Покрыты, в том числе, анализ рисков в комментариях, расчёт structured sentiment и оценка риска по feedback.

---

## База данных

- Миграции: **Flyway**, каталог `backend/src/main/resources/db/migration/`.
- Hibernate: `ddl-auto: validate` (схема только через Flyway).
- Версии миграций: пользователи и роли → HR/стажёр → уведомления → план и задачи → опросы v2 → шаблоны → подразделения → review workflow → sentiment (V16).

Подключение по умолчанию (локально):
...
Внутренний проект Naumen. По вопросам развёртывания и доступа к OpenRouter обращайтесь к команде разработки.
